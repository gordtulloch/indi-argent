...
#define Argent_TIMEOUT 6
#define Argent_BUF     64

// We declare an auto pointer to Argent.
static std::unique_ptr<Argent> Argent(new Argent());
...
const char *Argent::getDefaultName()
{
    return "Argent";
}

bool Argent::initProperties()
{
...
IPState Argent::updateWeather()
{
    char response[Argent_BUF];
    ...
    {
        int rc = -1;
        if ((rc = tty_read_section(PortFD, response, 0xA, Argent_TIMEOUT, &nbytes_read)) != TTY_OK)
        {
            char errstr[MAXRBUF];
...
Argent::AckResponse Argent::ack()
{
    char response[Argent_BUF] = {0};
    int nbytes_read = 0;

    if (isSimulation())
    {
        strncpy(response, "Argent by Gord Tulloch\r\n", 64);
        nbytes_read = strlen(response);
    }
    else
    {
...
bool Argent::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
...
bool Argent::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
...
bool Argent::getCalibration(bool sendCommand)
{
...
bool Argent::setCalibration(CalibrationType type)
{
...
bool Argent::verifyCRC(const char *response)
{
    ...
    char checksum_string[Argent_BUF] = {0};
    strncpy(checksum_string, response + 1, Argent_BUF);
...
///
//////////////////////////////////////////////////////////////////////
std::vector<std::string> Argent::split(const std::string &input, const std::string &regex)
{
